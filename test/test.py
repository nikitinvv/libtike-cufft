...
if __name__ == "__main__":
    ...
    prbamp = cp.array(dxchange.read_tiff(
        'test/prb217_amp.tiff').astype('float32'))
    prbang = cp.array(dxchange.read_tiff(
        'test/prb217_ang.tiff').astype('float32'))
    ...
    psiamp = cp.array(dxchange.read_tiff(
        'test/initpsiamp.tiff').astype('float32'))
    psiang = cp.array(dxchange.read_tiff(
        'test/initpsiang.tiff').astype('float32'))
    ...
    prb = prb.swapaxes(1, 2)  # Initial guess for the probe can be estimated
