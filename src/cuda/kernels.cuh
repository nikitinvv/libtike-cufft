						 ...
						 int Ntheta, int Nz, int N, int Nscan, int Nprb, int Ndetx, int Ndety)
{
	...
	g[shift + ix + iy * Ndetx + ty * Ndetx * Ndety + tz * Ndetx * Ndety * Nscan].x = c * prb0.x * f0.x - c * prb0.y * f0.y;
	g[shift + ix + iy * Ndetx + ty * Ndetx * Ndety + tz * Ndetx * Ndety * Nscan].y = c * prb0.x * f0.y + c * prb0.y * f0.x;
}
...
void __global__ mulaprobeq(float2 *prb, float2 *g, float2 *f, float *scanx, float *scany,
						   int Ntheta, int Nz, int N, int Nscan, int Nprb, int Ndetx, int Ndety)
{
	int tx = blockDim.x * blockIdx.x + threadIdx.x;
	int ty = blockDim.y * blockIdx.y + threadIdx.y;
...
void __global__ shifts(float2 *f, float2 *shiftx, float2 *shifty, int dir, int Ntheta, int Nscan, int NdetxNdety)
{
	...
	f0 = f[ind];
	f[ind].x = f0.x * shifty0.x - f0.y * shifty0.y * dir;
	f[ind].y = f0.y * shifty0.x + f0.x * shifty0.y * dir;
}
